...
theta = np.array(
    [0.132, 0.0001811, 0.561, 0.0178, 0.0018, 9.554, 1.56, 0.0932, 0.00461, 0.0000102, 0.0000443, 934.177, 33.347,
     17.144])

time_np = np.arange(54.4, 92.04, 0.01)
...
class FCN():
    ...
    def loss_data(self, t, u, v, w):
        ...
        print(self.dnn(t)[:, 0].reshape(-1, 1).size())
...
for epoch in range(num_iter + 1):
    ...
    with torch.no_grad():
        ...
        PINN.yt0[:] = PINN.yt0.clamp(1, 100.0)
        PINN.zt0[:] = PINN.zt0.clamp(1, 100.0)
...
# theta_df.to_csv('PINN_LMCI_out_theta.txt', sep='\t', encoding='utf-8', index=False, header=True)
