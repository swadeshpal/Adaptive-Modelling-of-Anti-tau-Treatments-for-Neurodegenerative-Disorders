...
u1 = zeros(1, N_T+1);     % Control input for government intervention
u2 = zeros(1, N_T+1);     % Control input for testing
...
L3(N_T+1) = c2;
  ...
  while(test < 0)
      ...
      for i = 1:N_T
          ...
          m11     = -k*x1(i)^2-k*x1(i)*x2(i)-ks*x1(i)*x3(i)-m1*x1(i)+lambda;
          m12     = (k/2)*x1(i)^2-k*x1(i)*x2(i)-k*x2(i)^2-ks*x2(i)*x3(i)-m2*x2(i)-u1(i)*x2(i);
          m13     = (k/2)*x2(i)^2+k*x1(i)*x2(i)-m3*x3(i)-u2(i)*x3(i);
      ...
      for i = 1:N_T % From initial to final value
          ...
          n11     = (2*L1(j)-L2(j))*k*x1(j) + (L1(j)+L2(j)-L3(j))*k*x2(j) + L1(j)*(ks*x3(j)+m1);
          n12     = (L1(j)+L2(j)-L3(j))*k*x1(j) + (2*L1(j)-L2(j))*k*x2(j) + L2(j)*(ks*x3(j)+m2+u1(j));
          n13     = ks*(L1(j)*x1(j)+L2(j)*x2(j)) + L3(j)*(m3+u2(j));
      ...
      U1 = min(u1_mx, max(0, L2.*x2./(b1)));
      u1 = 0.01.*U1 + 0.99.*oldu1;
      U2 = min(u2_mx, max(0, L3.*x3./(b2)));
...
%       J2    = Cost1 + Cost2 + Cost3 + Cost4 + Cost5;  % Cost at each time for ...plotting graphs
      ...
      test = min([temp1 temp2 temp4 temp5 temp6 temp11 temp12 temp13]);
  end
